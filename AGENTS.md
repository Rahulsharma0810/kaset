...
> ⚠️ **The app is sandboxed — most ad-hoc logging silently fails.** `Logger`/`os_log` `.info`/`.debug` lines do **not** reliably surface in `log stream`/`log show`, and a hardcoded `/tmp/...` file write is blocked by the sandbox and fails with no error. For throwaway diagnostics, write to **`NSTemporaryDirectory()`** (the app's container tmp), `synchronize()` after each line, and read it from the active container path: `~/Library/Containers/com.sertacozercan.Kaset/Data/tmp/` (for release) or `~/Library/Containers/com.sertacozercan.KasetLocal/Data/tmp/` (for local debug). Macro-level: window-screenshot automation is also unreliable here, so prefer file traces over visual capture. Always strip diagnostic instrumentation before commit.
...
🔐 **Keychain vs. File-based Cookie Storage**: Sandboxed macOS apps without a valid Developer ID or provisioning profile (such as local debug or ad-hoc builds) will hang inside `SecItemCopyMatching` when querying the Keychain. To bypass this, in `#if DEBUG` builds the app is compiled with bundle ID `com.sertacozercan.KasetLocal` (app name `KasetLocal`), stores cookies in the sandboxed Application Support folder under `Kaset/cookies.dat`, and completely bypasses Keychain API calls. This custom bundle ID also prevents Sparkle auto-update from replacing the debug app in the background.
